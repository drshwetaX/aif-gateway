/**
 * Author: Shweta Shah
 * Purpose: Read latest request events from Redis stream for Platform UI.
 * Endpoint: GET /api/redis/requests?limit=50
 */
import type { NextApiRequest, NextApiResponse } from "next";
import { getRedis } from "@/lib/redis";

const REDIS_PREFIX = process.env.AIF_REDIS_PREFIX || "aif";
const KEY = `${REDIS_PREFIX}:requests`;

function normalizeEntries(entries: any[]) {
  // Upstash returns: [ [id, {field:value}], ... ]
