 ...
 * Date: 2026-01-27
 * Purpose: Pause agent (runtime control).
 */
...
export default function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  const a = updateAgent(id, { status: "paused" });
  if (!a) return res.status(404).json({ error: "Agent not found" });

  appendAudit({ ts: Date.now(), type: "agent_pause", user_hash: user, data: redact({ agent_id: id, agent_status: a.status }) });
